...
type PaperclipRuntimeSkillEntry = {
  key: string;
  runtimeName: string;
  source: string;
  required: boolean;
...
function readEnvHome(config: Record<string, unknown>): string {
  const env = parseObject(config.env);
  return path.resolve(asString(env.HOME) ?? os.homedir());
}

export function resolveKiloSkillsHome(config: Record<string, unknown>): string {
  return path.join(readEnvHome(config), ".kilo", "skills");
}

function normalizeRuntimeSkills(value: unknown): PaperclipRuntimeSkillEntry[] {
  if (!Array.isArray(value)) return [];

  const out: PaperclipRuntimeSkillEntry[] = [];
...
function readSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
...
function canonicalizeDesiredSkillReference(
  reference: string,
  ...
  const runtimeMatches = availableEntries.filter((entry) =>
    typeof entry.runtimeName === "string" && entry.runtimeName.trim().toLowerCase() === normalized,
  );
  if (runtimeMatches.length === 1) return runtimeMatches[0]?.key ?? normalized;

  const slugMatches = availableEntries.filter((entry) =>
    entry.key.trim().toLowerCase().split("/").pop() === normalized,
  );
  if (slugMatches.length === 1) return slugMatches[0]?.key ?? normalized;
...
async function readInstalledSkillTargets(skillsHome: string): Promise<Map<string, InstalledSkillTarget>> {
...
async function ensurePaperclipSkillSymlink(source: string, target: string): Promise<"created" | "repaired" | "skipped"> {
  ...
  if (resolvedLinkedPath === path.resolve(source)) return "skipped";
...
function originFor(entry: PaperclipRuntimeSkillEntry): Pick<AdapterSkillEntry, "origin" | "originLabel" | "readOnly"> {
  if (entry.required) {
    return {
...
async function buildKiloSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  for (const available of availableEntries) {
    ...
    let managed = false;
    let state: AdapterSkillState = "available";
...
export async function ensureKiloSkillsInjected(
  config: Record<string, unknown>,
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[paperclip] Failed to prepare Kilo skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const entry of selectedEntries) {
    try {
      ...
      await onLog(
        "stderr",
        `[paperclip] ${result === "repaired" ? "Repaired" : "Linked"} Kilo skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
