...
export const fallbackModels: Array<{ id: string; label: string }> = [
...
export function parseKiloModelsOutput(output: string): Array<{ id: string; label: string }> {
...
export async function listKiloModels(command = "kilo"): Promise<Array<{ id: string; label: string }>> {
  ...
  if ((result.exitCode ?? 1) !== 0) {
    ...
    const candidateDirs: string[] = [homedir()];
